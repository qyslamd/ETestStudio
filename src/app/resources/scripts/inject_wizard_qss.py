...
TOPOLOGY_FILE_START = "/* ===== TopologyFileWizard 向导 (自动生成, 勿手改) START ===== */"
TOPOLOGY_FILE_END = "/* ===== TopologyFileWizard 向导 (自动生成, 勿手改) END ===== */"
...
def build_topology_file_block(theme):
    ctx = theme_context(theme)
    accent = ctx["accent"]
    accent_light = ctx["accent_light"]
    text = ctx["text"]
    ...
    danger = "#D13438"
    danger_light = "rgba(209, 52, 56, 0.14)"

    conn_combos = ",\n".join(
        "QComboBox#%s" % n
        for n in ["topoConnDeviceCombo", "topoConnDevicePortCombo",
                  "topoConnUutCombo", "topoConnUutPortCombo"])
    ...
    lines = [
        ...
        "  background-color: %s;" % ghost_hover,
        "}",
        "#topoItemBtn[danger=\"true\"]:hover {",
        "  background-color: %s;" % danger_light,
        "}",
        "#topoPortChip {",
        "  background-color: %s;" % grid_bg,
        "  border-radius: 6px;",
...
def main():
    ...
    blocks = [
        ...
        (TOPOLOGY_FILE_START, TOPOLOGY_FILE_END, build_topology_file_block),
    ]
