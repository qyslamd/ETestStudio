...
bool RunConfig::fromJson(const QJsonObject& obj) {
  ...
  for (const auto& v : monitorsArr) {
    ...
    Monitor m;
    m.connectionId = mo[QStringLiteral("connectionId")].toString();
  ...
  for (const auto& v : layoutArr) {
    ...
    LayoutItem l;
    l.connectionId = lo[QStringLiteral("connectionId")].toString();
...
bool RunConfigEditor::save() {
  ...
  if (path.isEmpty()) {
    path = QFileDialog::getSaveFileName(
        this, QStringLiteral("保存运行配置"), QString(),
...
void RunConfigEditor::initUi() {
  ...
  // 工具栏（骨架：新建/保存；排列/分布后续随主视图接入）
  toolbar_ = new QToolBar(QStringLiteral("运行配置"), this);
  toolbar_->setObjectName(QStringLiteral("runConfigToolbar"));
  toolbar_->setMovable(false);
  auto* newAction = toolbar_->addAction(QStringLiteral("新建"));
  newAction->setToolTip(QStringLiteral("新建运行配置"));
  ...
  auto* saveAction = toolbar_->addAction(QStringLiteral("保存"));
  saveAction->setToolTip(QStringLiteral("保存运行配置 (.erun)"));
  connect(saveAction, &QAction::triggered, this, [this]() { save(); });
  auto* addMonitorAction = toolbar_->addAction(QStringLiteral("添加监听器"));
  addMonitorAction->setToolTip(QStringLiteral("添加一个监听器卡片"));
  connect(addMonitorAction, &QAction::triggered, this,
  ...
  // 信息区
  file_label_ = new QLabel(QStringLiteral("未打开文件"), this);
  file_label_->setObjectName(QStringLiteral("runConfigFile"));
  test_program_label_ = new QLabel(QStringLiteral("测试程序: (未选择)"), this);
  test_program_label_->setObjectName(QStringLiteral("runConfigTestProgram"));

  auto* infoLayout = new QVBoxLayout();
  infoLayout->setContentsMargins(8, 8, 8, 4);
  infoLayout->addWidget(file_label_);
  infoLayout->addWidget(test_program_label_);
  vbox->addLayout(infoLayout);

  // 主视图：可视化区（编辑态，监听器卡片 + 手动布局）
  vis_area_ = new VisualizationArea(this);
  vis_area_->setEditMode(true);
  vbox->addWidget(vis_area_, 1);

  // 布局被改动（拖拽/resize/排列/分布）→ 置脏，关闭 tab 时提示保存
  connect(vis_area_, &VisualizationArea::layoutChanged, this,
          &RunConfigEditor::markModified);
...
void RunConfigEditor::refreshUi() {
  file_label_->setText(
      file_path_.isEmpty() ? QStringLiteral("未打开文件") : file_path_);
  test_program_label_->setText(
      QStringLiteral("测试程序: %1")
          .arg(config_.testProgram.isEmpty() ? QStringLiteral("(未选择)")
                                             : config_.testProgram));
