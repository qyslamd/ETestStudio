...
SignalSelectionDialog::SignalSelectionDialog(
  ...
  connect(node_tree_->selectionModel(), &QItemSelectionModel::currentChanged,
          this, &SignalSelectionDialog::onNodeSelected);
...
void SignalSelectionDialog::populateDevices() {
  ...
  LOG_DEBUG("UUID",
            "SignalSelectionDialog::populateDevices: found {} device ids",
            ids.size());
  for (const QString& id : ids) {
    QString name = registry_->deviceName(id);
    // 显示 "设备名 (id)"，userData 存 id
    QString label =
        name.isEmpty() ? id : QStringLiteral("%1 (%2)").arg(name, id);
...
void SignalSelectionDialog::populatePorts(const QString& deviceId) {
  ...
  QStringList ports;
  registry_->forEachPortBinding([&](const QString& dId, const QString& portName,
...
void SignalSelectionDialog::onPortChanged(int index) {
  ...
  if (index < 0 || device_combo_->currentIndex() < 0)
    return;
...
void SignalSelectionDialog::populateFrames(const QString& deviceId,
  ...
  registry_->forEachPortBinding([&](const QString& dId, const QString& pName,
                                    const QStringList& frameNames) {
...
void SignalSelectionDialog::populateNodes(const QString& frameName) {
  ...
  addChildren = [&](QStandardItem* parent, const icd::Node* node) {
    auto* item = new QStandardItem(QString::fromUtf8(
        node->name().data(), static_cast<int>(node->name().size())));
    // 存 node 指针供选中时查询
  ...
  for (const auto& root : frame->roots()) {
    auto* rootItem = new QStandardItem(QString::fromUtf8(
        root->name().data(), static_cast<int>(root->name().size())));
...
void SignalSelectionDialog::onNodeSelected(const QModelIndex& index) {
  ...
  if (device_combo_->currentIndex() < 0 || port_combo_->currentIndex() < 0 ||
  ...
  current_uuid_ = etest::core::SignalRegistry::computeUuid(deviceId, portName,
                                                           frameName, nodePath);
  updateInfoLabel();
  updateOkButton();
}
...
void SignalSelectionDialog::updateInfoLabel() {
  ...
  switch (vt) {
    ...
    case icd::ValueType::byte_:
      typeName = QStringLiteral("uint8");
      break;
    case icd::ValueType::bytes:
      typeName = QStringLiteral("bytes");
      break;
    case icd::ValueType::word:
      typeName = QStringLiteral("uint16");
      break;
    case icd::ValueType::shortint:
      typeName = QStringLiteral("int16");
      break;
    case icd::ValueType::smallint:
      typeName = QStringLiteral("smallint");
      break;
    case icd::ValueType::longword:
      typeName = QStringLiteral("uint32");
      break;
    case icd::ValueType::integer:
      typeName = QStringLiteral("int32");
      break;
    case icd::ValueType::ulong_:
      typeName = QStringLiteral("uint64");
      break;
  ...
  info_label_->setText(QStringLiteral("类型: %1 | Offset: %2 | Bit: %3~%4")
                           .arg(typeName)
                           .arg(node->offset())
                           .arg(node->bit_offset())
...
void SignalSelectionDialog::updateOkButton() {
  ...
  if (btn) {
    ...
    uuid_label_->setText(valid ? QStringLiteral("UUID: %1").arg(current_uuid_)
                               : QString());
  }
}
