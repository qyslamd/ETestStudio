...
> **本文档来源**：本文档基于《通用综合测试系统设计方案_V1.0.pdf》（78页，公司内部硬件厂商视角）演化而来，在保留原设计思想的基础上扩展了详细接口设计、关键技术方案、可视化编辑器设计、开发计划和验证方案等内容。原PDF从"我司"（硬件生产厂商）角度撰写，本文档调整为通用项目视角。
>
> **原文PDF位置**：`docs/01-规划/通用综合测试系统设计方案_V1.0.pdf`

**版本**: V2.0（基于PDF V1.0扩展）
**日期**: 2026年5月
**作者**: 研发团队
...
5. [各层详细设计](#5-各层详细设计)
6. [数据流设计](#6-数据流设计)
   1. [基础环境搭建流程](#611-硬件环境初始化)
   2. [运行时数据流（Engine↔硬件）](#62-运行时数据流)
7. [关键技术方案](#7-关键技术方案)
...
> **参考产品型号（源自《通用综合测试系统设计方案_V1.0》）**：
>
> | 品类 | 产品型号 |
> |------|----------|
> | 串口通讯 RS232/422/485 | USB: EP-H1276(X) \| cPCI/PXI: EP-H5276(X) \| PCI: EP-H6276(X) |
> | MIL-STD-1553B | USB: EP-H1273_1553B \| cPCI/PXI: EP-H5273(X) \| PCI: EP-H6273(X) |
> | CAN | USB: EP-H1278 \| cPCI/PXI: EP-H5278(X) \| PCI: EP-H6278(X) |
> | FlexRay | cPCI/PXI: EP-H5285 \| PCI: EP-H6285 |
> | A429 | USB: EP-H1272(3_429) \| cPCI/PXI: EP-H5272(X) \| PCI: EP-H6272(X) |
> | AD采集 | cPCI/PXI: EP-H5022/EP-H5023(X)/EP-H5024(x)/EP-H5026(X)/EP-H5033(X)/EP-H5035~9 \| PCI: EP-H6033(X) |
> | DA | cPCI/PXI: EP-H5172(X)/EP-H5177（任意波）/EP-H5173(X)/EP-H5175(X) |
> | TTL输出(DO) | cPCI/PXI: EP-H5125(X) |
> | 开关量 | cPCI/PXI: EP-H5121(X)/5131~7 |
> | PWM | cPCI/PXI: EP-H5127(X) |
> | 离散量 | cPCI/PXI: EP-H5141(X)~9 |
> | AFDX | USB: EP-H1281 \| cPCI/PXI: EP-H5281 \| PCI: EP-H6281 \| PCIe: EP-X6281 |
> | 1394B | cPCI/PXI: EP-H5277 |
...
IATP采用**5层松耦合架构**（源自《通用综合测试系统设计方案_V1.0》），外加**数据持久化层**作为各层的基础支撑。
...
> **架构说明**：原始设计文档《通用综合测试系统设计方案_V1.0》定义为5层架构（应用层/用例管理层/测试引擎层/ICD信号层/硬件抽象层），数据持久化层作为各层的基础支撑，当前阶段采用分散到文件的方式存储，后续计划使用数据库。本文档在保留此5层架构基础上，额外列出数据持久化层以便清晰描述。部分参考文档亦称此结构为"六层架构"（将数据持久化层计入）。
...
应用程序图标设计理念（源自《通用综合测试系统设计方案_V1.0》）：
...
> 本节内容源自《通用综合测试系统设计方案_V1.0》第6章，描述使用本平台进行测试前的基础环境准备流程。
...
```
系统启动 → 遍历插件目录 → 逐一加载共享库(DLL/SO)
    → 读取插件信息(型号/厂家/通道数等)
    → 调用硬件插件 selfTest() 完成上电自检
    → 在 UI 设备树上显示(厂家→分类→设备→通道)
    → 对外(其它模块)提供硬件信息
```

硬件环境是测试的基础：确定当前有哪些激励设备（硬件模块）、它们支持什么功能、有多少通道、有哪些功能函数可用。

#### 6.1.2 测试环境初始化

测试环境以一个**测试工程**为单位进行创建，主要包含三个步骤：

**步骤1：创建硬件拓扑**

硬件拓扑是描述激励设备（可收可发）与被测设备（UUT，与激励设备一样）物理上连接的关系，通常以通道为创建连接的单位。通过拓扑编辑器界面，为当前测试工程创建硬件连接关系拓扑图，输出硬件拓扑描述配置文件（如`.etopo`格式）。

**步骤2：编辑ICD帧协议**

帧协议用于描述信号长什么样、数据怎么解的关键文件。在一个测试系统中可能存在数个或数十个帧协议，它们会被ICD层的协议仓库加载解析，承担数据传输过程中的"翻译者"角色。输出帧协议描述文件。

**步骤3：编写测试用例**

基于已配置的硬件拓扑和帧协议，编写或生成测试用例（JSON/Excel/YML/Lua）。

#### 6.1.3 总结

| 要素 | 描述 | 产出 |
|------|------|------|
| 硬件环境 | 有哪些激励设备、支持什么功能、多少通道 | 设备树（系统自动完成） |
| 拓扑关系 | 激励设备和UUT之间物理上的数据通路 | 拓扑描述文件（`.etopo`） |
| 帧协议 | 信号怎么转换、解析、在哪条物理通路上传输 | 帧协议描述文件 |
| 测试用例 | 具体的测试步骤和判定条件 | 用例文件（JSON/Lua等） |
...
### 10.3 阶段2：HAL接口定义 + Mock实现（仅接口与模拟，无真实硬件）

**核心任务**：完成所有设备类型的插件接口定义和Mock模拟实现，构建设备树管理UI和自检流程。本阶段不涉及任何真实硬件对接，所有插件运行于Dry Run模式，为上层ICD/Engine开发提供模拟数据源。
...
| 2.12 | 设备自检流程            | 启动时统一自检（Mock模式下返回模拟状态）                          |
...
**说明**：本阶段将阶段2定义的插件接口对接真实硬件设备，实现从Dry Run模式到Real模式的切换。此时ICD信号层、用例管理层和测试引擎层已完成，可在真实硬件上跑通全链路测试。

**核心任务**：

| 编号 | 任务                       | 说明                                                               |
| ---- | -------------------------- | ------------------------------------------------------------------ |
| 6.1  | 真实设备插件实现           | 各设备类型从Mock实现替换/扩展为真实驱动实现                        |
| 6.2  | 设备发现与枚举             | 总线扫描、板卡识别、设备列表自动发现                               |
| 6.3  | VISA SCPI真实通信          | 基于visa32.dll进行真实仪器程控通信，*IDN?验证                      |
| 6.4  | 串口族真实驱动             | RS232/422/485基于Windows API的串口通信实现                         |
| 6.5  | CAN/CAN FD真实驱动         | 基于PCAN/Vector等硬件驱动的CAN通信实现                             |
| 6.6  | ARINC 429真实驱动          | 基于专用板卡API的429通信实现                                       |
| 6.7  | 1553B真实驱动              | 基于板卡SDK实现BC/RT/MT模式通信                                   |
| 6.8  | DA/DIO/Pulse真实驱动       | 基于板卡API的DA输出/开关量IO/脉冲信号实现                         |
| 6.9  | Dry Run ↔ Real模式切换     | 每个插件可独立切换模拟/真实模式，混搭运行                          |
| 6.10 | 真实设备自检               | 自检流程对接真实设备状态查询，区分在线/离线/故障                   |
| 6.11 | 硬件配置UI                 | 设备参数配置界面（总线地址/板卡号/通道映射等）                     |
| 6.12 | HAL与ICD全链路联调         | 真实设备→ICD解包→CVT→脚本验证的完整闭环调试                       |

### 10.8 阶段7：测试与优化

| 编号 | 任务            | 说明                           |
| ---- | --------------- | ------------------------------ |
| 7.1  | 全模块回归测试  | 各模块独立单元测试             |
| 7.2  | 全链路集成测试  | HAL→ICD→用例→引擎→报告闭环验证 |
| 7.3  | 兼容性测试      | Windows 10/11兼容性验证        |
| 7.4  | 压力/稳定性测试 | 长时间运行、大量信号、大量用例 |
| 7.5  | 用户体验优化    | 界面交互优化、性能优化         |
| 7.6  | 打包与文档      | 安装包、用户手册、开发文档     |
