...
| 开始 | 文件 panel：新建项目/打开项目/新建文件/打开文件（大按钮，tab 最左第一） | Office「开始」惯例：无项目时的引导入口；动作使能随项目状态 |
...
| D2 | 上下文页在「开始」页之后 | 编辑命令是高频区，紧随 Office 惯例的「开始」首 tab |
...
| D14 | 上下文页 insert 到 index 1（「开始」页之后）后立即 hide，确保 showCategory 恢复位置正确 | showCategory 恢复隐藏时记录的 index，须先固定；「开始」保持第一 tab |
