    ...
    static QPainterPath generateSmoothCurve(const QVector<QPointF> &points) {
        ...
        while (i < pointSize) {
            if (qIsNaN(points.at(i).y()) || qIsNaN(points.at(i).x()) || qIsInf(points.at(i).y())) {
                auto points1 = points.toStdVector();
                std::vector<QPointF> points2(points1.cbegin() + segmentStart, points1.cbegin() + i - segmentStart);
                QVector<QPointF> lineData(QVector<QPointF>::fromStdVector(points2));
...
public:
  ...
  */
  ...
  inline void setSmooth(bool smooth) { mSmooth = smooth; };
