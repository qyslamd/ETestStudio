...
TEST(ConfigManagerTest, ReadWrite)
{
    ...
    EXPECT_EQ(config.get<QString>(CONFIG_RECENT_LAST_OPEN_PATH), QString("D:/test/path"));
...
TEST(ConfigManagerTest, DefaultValue)
{
    ...
    EXPECT_EQ(config.get<bool>(CONFIG_BACKUP_ENABLED), CONFIG_BACKUP_DEFAULT_ENABLED);
    EXPECT_EQ(config.get<int>(CONFIG_BACKUP_INTERVAL_MIN), CONFIG_BACKUP_DEFAULT_INTERVAL_MIN);
}
