...
- **关于测试程序的思考**
  现在多个测试程序之间是相互独立的。一方面，我在一开始规划的时候，计划了使用Lua来和C++交互，用Lua脚本编写测试程序的。另一方面，我觉得测试程序就应该是一体的或者说是一套的，就像我们使用编程语言来编写一个应用程序一样，我们需要一个主程序(如C语言中的main函数)，然后可以编写一些其它的模块，通过控制流(if while loop等)来决定测试程序的走向，是不是可以考虑新增一个call命令，用于调用非主程序外的子程序？我觉得我们可以讨论讨论。

- **对于监听器这个角色的思考**
  背景：现在的监听器被融入进去了运行配置。由 RunConfigEditor 进行配置，由 ExecutionDashboard 消费(加载显示)。关于监听器或者说visualizer来说，它的职责是用于展示 拓扑 中某一条连线上传输的数据。引擎在执行的过程中会把数据从推送到visualizer中进行显示。
  那么我真的的思考是什么呢？
  我想让运行配置变成一个"独立的程序"，什么是独立的程序呢？就像我使用QtCreator来创建一个QWidget的窗体一样，我可以从控件列表(control palette)中拖动控件进来，然后设置布局，属性，然后连接信号和槽处理事件。
  QtCreator，准确来说是QtDesigner，就是这样的模式，它也是提供了一个场景，场景中是一个窗口(标题栏和边框)，所有从控件列表拖放进入的控件，都将放在这个窗体上。然后用户选中控件之后可以修改它的属性(当然它有很多很多的属性)。然后保存到后缀为.ui的文件中去。
  那我们现在的 RunConfigEditor 借鉴这个思想，我们要做什么呢？
  1. 改造我们的场景，也提供一个类似于窗口的东西，所有的visualizer只能放在这个窗口范围内，这样做，得到的好处就是，当用户加载一个编辑好的etproj工程之后，打开运行，就像是一个应用程序一样！
  2. 现在可能不做但是可以头脑风暴的想法：和测试程序联动，测试程序中的参数可以用占位的方式预留，这个想法就不仅仅是监听器了，做了类似于labview去了。是不是很有意思？
  当然了，这个想法我们可以讨论了记录到文档中，不一定要做，这是一个比较宏大的设计，也许和我们现有的代码冲突非常大，它的改造影响面很大，而且有很多细节需要推敲

- **信号编辑器**
  我觉得我们需要一个信号编辑器。它在项目树中是一个独立的分类，用于定于信号和ICD的关联，这样在拓扑对连线进行信号定义的时候，正好可以用这个信号的定义，也能很好的对信号起别名
---
